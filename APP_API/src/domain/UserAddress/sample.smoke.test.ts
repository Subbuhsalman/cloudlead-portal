...
//         .post('/api/userAddress') // Adjust path as necessary
//         .send({ name: 'Test Entity', description: 'Test Description' }); // UserAddress payload

//       expect(res.statusCode).toBe(201);
//       expect(res.body).toHaveProperty('id');
//       userAddressId = res.body.id; // Store for later tests
//     });
//   });
...
//       const res = await request(app)
//         .put(`/api/userAddress/${userAddressId}`)
...
//       const res = await request(app).delete(`/api/userAddress/${userAddressId}`);
...
//       const res = await request(app).get(`/api/userAddress/${userAddressId}`);
...
//       const res = await request(app).get(`/api/userAddress/${newEntity.body.id}`);
//       expect(res.statusCode).toBe(200);
...
//       const res = await request(app).get('/api/userAddress/list/paginated?page=1&size=10');
//       expect(res.statusCode).toBe(200);
...
//   describe('GET /', () => {
//     it('should return a list of userAddress and status 200', async () => {
//       const res = await request(app).get('/api/userAddress');
//       expect(res.statusCode).toBe(200);
