...
//         .post('/api/coupon') // Adjust path as necessary
//         .send({ name: 'Test Entity', description: 'Test Description' }); // Coupon payload

//       expect(res.statusCode).toBe(201);
//       expect(res.body).toHaveProperty('id');
//       couponId = res.body.id; // Store for later tests
//     });
//   });
...
//       const res = await request(app).get(`/api/coupon/${couponId}`);
...
//       const res = await request(app).get(`/api/coupon/${newEntity.body.id}`);
//       expect(res.statusCode).toBe(200);
...
//       const res = await request(app).get('/api/coupon/list/paginated?page=1&size=10');
//       expect(res.statusCode).toBe(200);
...
//   describe('GET /', () => {
//     it('should return a list of coupon and status 200', async () => {
