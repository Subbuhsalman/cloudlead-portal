...
//         .post('/api/orders') // Adjust path as necessary
//         .send({ name: 'Test Entity', description: 'Test Description' }); // Orders payload

//       expect(res.statusCode).toBe(201);
//       expect(res.body).toHaveProperty('id');
//       ordersId = res.body.id; // Store for later tests
//     });
//   });
...
//       const res = await request(app).delete(`/api/orders/${ordersId}`);
...
//       const res = await request(app).get(`/api/orders/${ordersId}`);
...
//       const res = await request(app).get(`/api/orders/${newEntity.body.id}`);
//       expect(res.statusCode).toBe(200);
...
//       const res = await request(app).get('/api/orders/list/paginated?page=1&size=10');
//       expect(res.statusCode).toBe(200);
