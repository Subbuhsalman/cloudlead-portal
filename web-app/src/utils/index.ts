...
// import {jwtDecode} from 'jwt-decode'; // Currently unused, but may be needed for JWT token expiry handling
...
export const saveCookieData = (data:DataType)=>{
  ...
  {
    cookies.set(localKey, value, { path: '/', secure:secure,sameSite:sameSite,domain:COOKIE_DOMAIN});    // const {exp} = jwtDecode<any>(value);
...
export const maskEmail = (email: string, keep = 7, mask = '***'): string =>{
  const [local, domain] = email.split('@');
  if (!domain) return email; // not a valid email, just return it

  const shown = local.slice(0, Math.min(keep, local.length));
