...
class useHttp {
        ...
        this.api.interceptors.response.use(function (response) {
            ...
            try {
                ...
                // accessToken if token valid or not
                    ...
                    const url: string = BASE_URL + "/auth/refresh-token";
                    await axios.post(url, {}, { headers: { 'refresh-token': getCookieData({ localKey: "refreshToken" }) } })
                        .then((response) => {
    ...
    public post(endpoint: string, postData: object) {
        return this.api.post(endpoint, postData);
    }

    public put(endpoint: string, postData: object) {
        return this.api.put(endpoint, postData);
    }
    ...
    public getV2(endpoint: string, config: object) {
        return this.api.get(endpoint, !config ? {} : config);
    }

    public postV2(endpoint: string, postData: object, config: object) {
        return this.api.post(endpoint, postData, !config ? {} : config);
    }

    public putV2(endpoint: string, postData: object, config: object) {
        return this.api.put(endpoint, postData, !config ? {} : config);
    }

    public deleteV3(endpoint: string, config: object) {
        return this.api.delete(endpoint, !config ? {} : config);
    }
