...
import { publicPaths } from "@/utils/menuItem";
...
const useLoginDetails = () => {
  ...
  const registerUserHook = React.useCallback((formData:any, onSuccess:any) => {
    dispatch(registerUser(formData, onSuccess));
  }, [dispatch]);
