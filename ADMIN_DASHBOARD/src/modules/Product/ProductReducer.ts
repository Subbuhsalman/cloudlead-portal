import { ProductInitalState } from "./ProductInitalState";
import {
  PRODUCT_LOADING,
  PRODUCT_FORM_SUBMIT,
  PRODUCT_FORM_HTTP_REQUEST,
  PRODUCT_ADD,
  PRODUCT_GET_PAGINATED_LIST,
  PRODUCT_DELETE,
  PRODUCT_PAGNIATION_UPDATE,
  PRODUCT_SUCCESS_TOAST,
  PRODUCT_GET,
  PRODUCT_LAYOUT_STYLE,
  PRODUCT_UPDATE,
  PRODUCT_IS_INITIAL_TABLE_DATA_LOADED_UPDATE
} from "./ProductTypes";

const ProductReducer = (state: any = ProductInitalState.Product, action: any): any => {
  switch (action.type) {
    case PRODUCT_SUCCESS_TOAST:
      return { ...state, ...action.payLoad };

    case PRODUCT_FORM_SUBMIT:
      return { ...state, ...action.payLoad };

    case PRODUCT_FORM_HTTP_REQUEST:
      return { ...state, ...action.payLoad };

    case PRODUCT_LOADING:
      return { ...state, ...action.payLoad };

  case PRODUCT_IS_INITIAL_TABLE_DATA_LOADED_UPDATE:
        return { ...state, ...action.payLoad };
      
    case PRODUCT_ADD:
      {
        const tableList = state?.list;

        if (state.per_page === parseInt(tableList?.length)) {
          const startIndex = tableList?.length - 1; // Calculate the starting index
          tableList?.splice(startIndex, 1); // Get a new array with the removed elements
        }
  
        return {
          ...state,
          list: [...[action.payLoad], ...tableList],
          total_records: action.total_records,
        };
      }
    

    case PRODUCT_GET_PAGINATED_LIST:
      return { ...state, ...action.payLoad };

    case PRODUCT_PAGNIATION_UPDATE:
      return { ...state, ...action.payLoad };

    case PRODUCT_LAYOUT_STYLE:
      return { ...state, ...action.payLoad };

    case PRODUCT_UPDATE:
      {
        { const { record, listIndex } = action.payLoad;
        const itemData: any = state.list[listIndex];
        state.list[listIndex] = { ...itemData, ...record };
        return state; }
      }
      

    case PRODUCT_GET:
      return { ...state, ...action.payLoad };

    case PRODUCT_DELETE:
      {
        const Data = state.list.filter(
          (listItem: any) =>
            listItem.product_id !== action.payLoad.product_id
        );
        return { ...state, list: [...Data], total_records: action.total_records };
      }
     
  }

  return state;
};

export { ProductReducer };
